...
export class CameraExample extends Scene {
  ...
  create() {
    ...
    let camera: Camera | null = this.add.existing(new Camera(this)).move(vec2(this.halfWidth(), this.halfHeight()));

    this.app().controlCallback(`Destroy Camera`, () => {
      if (camera !== null) {
        camera?.removeFromDisplayList().removeFromUpdateList().destroy();
        camera = null;
      }
    });

    this.app().controlCallback(`Create Camera`, () => {
      if (camera === null) {
        camera = new Camera(this).move(vec2(this.halfWidth(), this.halfHeight()));
      }
    });

    this.app().controlCallback(`Add Camera to Scene`, () => {
      if (camera !== null) {
